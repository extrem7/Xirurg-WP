...
function print_breadcrumbs() {
	global $lang;
	/* === ОПЦИИ === */
	$text['home']     = get_the_title( pll_get_post( 8, $lang ) ); // текст ссылки "Главная"
	...
	$wrap_before    = '<li class="breadcrumb-item">'; // открывающий тег обертки
	$wrap_after     = '</li>'; // закрывающий тег обертки
	...
	if ( is_home() || is_front_page() ) {
		...
		if ( is_category() ) {
			...
			if ( get_post_type() != 'post' ) {
				...
				/**/
				if ( get_post_type() == 'operation' ) {
					$term = get_the_terms( $post, 'type' )[0];
					if ( $term ) {
						if ( get_field( 'ссылка', $term ) ) {
							printf( $link, $home_url . $slug['slug'] . '/', $post_type->labels->singular_name );
						} else {
							printf( $link, $home_url . $slug['slug'] . '/', $post_type->labels->singular_name );
							echo $sep;
							printf( $link, get_term_link( $term, 'type' ), $term->name );
						}
					} else {
						printf( $link, $home_url . $slug['slug'] . '/', $post_type->labels->singular_name );
					}
			...
			if ( get_query_var( 'paged' ) ) {
				if ( get_post_type() == 'operation' && is_tax() ) {
					$cat = get_queried_object();
					echo $sep . sprintf( $link, get_post_type_archive_link( $post_type->name ), $post_type->label ) . $sep . sprintf( $link, get_term_link( $cat->term_id, 'type' ), $cat->name ) . $sep . $before . sprintf( $text['page'], get_query_var( 'paged' ) ) . $after;
				} else {
				...
				if ( get_post_type() == 'operation' && is_tax() ) {
					$cat = get_queried_object();
					echo $sep;
					printf( $link, get_post_type_archive_link( $post_type->name ), $post_type->label );
					echo $sep . $before . $cat->name . $after;
				} else {
					if ( $show_current ) {
